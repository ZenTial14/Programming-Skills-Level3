...
def choose_state(states):
    while True:
        ...
        origin_state = input('Choose the state from where you want to send the shipment: ')
        destiny_state = input('Choose the state for the destination: ')
        if origin_state == 'back' or destiny_state == 'back':
            ...
            amount = input('¿How many packages you want to send? [Min 100/ Max 500]: ')
            if 100 <= int(amount) <= 500:
                system('cls')
                price = calculate_price(origin_state, destiny_state, states, int(amount))
